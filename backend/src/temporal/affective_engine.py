...
class AffectiveEngine:
    ...
    def __init__(self, thresholds: EmotionThresholds = EmotionThresholds()) -> None:
        self.th = thresholds
    ...
    def _detect_excitement(self, state: AgentState) -> EmotionReading:
        ...
        return EmotionReading(label=EmotionLabel.EXCITEMENT, intensity=intensity,
    ...
    def _detect_enthusiasm(self, state: AgentState) -> EmotionReading:
        ...
        return EmotionReading(label=EmotionLabel.ENTHUSIASM, intensity=intensity,
    ...
    def _detect_contentment(self, state: AgentState) -> EmotionReading:
        ...
        return EmotionReading(label=EmotionLabel.CONTENTMENT, intensity=intensity,
    ...
    def _detect_frustration_emo(self, state: AgentState) -> EmotionReading:
        ...
        return EmotionReading(label=EmotionLabel.FRUSTRATION_EMO, intensity=intensity,
                              description="Goals are blocked; agitated negative state.")

    def _detect_anxiety(self, state: AgentState, psi: PersonalityVector,
                        reg: DimensionRegistry) -> EmotionReading:
        """High arousal + negative mood + high N."""
        N = psi["N"] if "N" in reg.keys else 0.5
        intensity = state.arousal * max(0, -state.mood) * N
        return EmotionReading(label=EmotionLabel.ANXIETY, intensity=intensity,
                              description="Tense anticipation of negative outcomes.")

    def _detect_boredom(self, state: AgentState, memory: MemoryBank,
                        psi: PersonalityVector, reg: DimensionRegistry) -> EmotionReading:
        """Low arousal + low outcome variance + understimulation."""
        O = psi["O"] if "O" in reg.keys else 0.5
        E = psi["E"] if "E" in reg.keys else 0.5
        desired_arousal = 0.3 + 0.25 * O + 0.2 * E
        ...
        return EmotionReading(label=EmotionLabel.BOREDOM, intensity=intensity,
                              description="Understimulated; craving novelty or challenge.")

    def _detect_melancholy(self, state: AgentState, memory: MemoryBank,
                           psi: PersonalityVector, reg: DimensionRegistry) -> EmotionReading:
    ...
    def _detect_saudade(self, state: AgentState, memory: MemoryBank,
        ...
        intensity = float(np.clip(
            (gap - self.th.saudade_gap_threshold) * personality_amplifier * (1 + low_arousal), 0, 1))
        return EmotionReading(label=EmotionLabel.SAUDADE, intensity=intensity,
    ...
    def _detect_regret(self, memory: MemoryBank) -> EmotionReading:
        ...
        return EmotionReading(label=EmotionLabel.REGRET, intensity=intensity,
                              description="Counterfactual awareness: unchosen paths were better.")

    def _detect_perseverance(self, memory: MemoryBank, still_acting: bool) -> EmotionReading:
        ...
        if not still_acting or failures < self.th.perseverance_failure_threshold:
            return EmotionReading(label=EmotionLabel.PERSEVERANCE, intensity=0.0,
                                  description="Not persevering.")
        intensity = float(np.clip(
            (failures - self.th.perseverance_failure_threshold + 1) * 0.2, 0, 1))
        return EmotionReading(label=EmotionLabel.PERSEVERANCE, intensity=intensity,
                              description="Continuing despite repeated failure.")

    def _detect_grit(self, memory: MemoryBank, still_acting: bool) -> EmotionReading:
        """Sustained perseverance over extended adversity."""
        failures = memory.consecutive_failures()
        if not still_acting or failures < self.th.grit_failure_threshold:
            return EmotionReading(label=EmotionLabel.GRIT, intensity=0.0,
                                  description="Not showing grit.")
        intensity = float(np.clip(
            (failures - self.th.grit_failure_threshold + 1) * 0.15, 0, 1))
        return EmotionReading(label=EmotionLabel.GRIT, intensity=intensity,
                              description="Sustained perseverance over extended adversity.")

    def _detect_creativity(self, activations: np.ndarray, state: AgentState,
                           reg: DimensionRegistry) -> EmotionReading:
        """High O-activation + adequate energy + moderate arousal (Yerkes-Dodson)."""
        if "O" not in reg.keys:
            return EmotionReading(label=EmotionLabel.CREATIVITY, intensity=0.0,
        ...
        return EmotionReading(label=EmotionLabel.CREATIVITY, intensity=intensity,
                              description="Creative capacity: open activation + optimal arousal + energy.")

    def _detect_focus(self, activations: np.ndarray, state: AgentState,
                      reg: DimensionRegistry) -> EmotionReading:
        """High C-activation + high energy + low frustration."""
        if "C" not in reg.keys:
            return EmotionReading(label=EmotionLabel.FOCUS, intensity=0.0,
                                  description="No C dimension.")
        c_act = activations[reg.index("C")]
        intensity = float(np.clip(c_act * state.energy * (1 - state.frustration * 0.7), 0, 1))
        return EmotionReading(label=EmotionLabel.FOCUS, intensity=intensity,
                              description="Attentional control: conscientious activation + energy - frustration.")
